...
def classify(inX,dataSet,labels,k):  # inX 测试数据 dataSet 训练数据 labels 标签项
    ...
    for i in range(k):
        ...
        sortedClassCount=sorted(classCount.items(),key=operator.itemgetter(1),reverse=True)  # 降序排序 获取第一个值  即是获得频率最高值
